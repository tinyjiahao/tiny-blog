...
降级发生在「实时调用失败」之后，这时你手上只有**原始请求参数**，所以缓存 key 必须能用请求参数现拼出来。这里有个生产里很典型的案例——**类似大众点评这种强依赖 LBS（地理位置）的搜索服务**。
...
一个用户在「国贸商圈」搜「火锅」，坐标 116.40421, 39.91521：
  ...
  search:火锅:116.40421:39.91521  →  [店A, 店B, 店C]   ← 保留 5 位（亚米级）
  search:火锅:116.4042:39.9152    →  [店A, 店B, 店C]   ← 保留 4 位（几十米）
  search:火锅:116.404:39.915      →  [店A, 店B, 店C]   ← 保留 3 位（约百米）
...
写入时（一次请求构建多个粒度的 key）：
  ...
  真实结果 [店A, 店B, 店C]
        ↓ 计算内容指纹
  uuid = hash([店A, 店B, 店C]) = "r7k3x9"
        ↓ 多个逻辑 key 都指向同一个 uuid
  search:火锅:116.40421:39.91521  ──┐
  search:火锅:116.4042:39.9152    ──┼──►  uuid:r7k3x9  ──►  [店A, 店B, 店C]
