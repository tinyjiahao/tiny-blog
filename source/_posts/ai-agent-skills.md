...
上面是片段，这里给一个**完整、自包含、可直接复制使用**的 Skill。它的任务很具体：**按约定式提交规范生成 commit message**（Conventional Commits）。这是个几乎所有开发者都用得上的小技能，结构简单但五脏俱全。

**目录结构**（演示 references 拆分）：

```
commit-message/
├── SKILL.md              ← 主文件（工作流程 + 规则 + 示例）
└── references/
    └── conventions.md    ← 各类型详细规范（按需读）
```
...
---
name: commit-message
description: 生成符合约定式提交规范（Conventional Commits）的 commit message。
  当用户说「提交代码」「写个 commit」「生成提交信息」「帮我写 commit message」、
  或在执行 git commit 前需要组织提交信息时使用。即使用户没明说规范，
  也按约定式提交格式生成（feat/fix/docs/chore 等前缀）。
---

# 生成 commit message

按约定式提交规范（Conventional Commits）生成提交信息，让 git 历史清晰、
可自动生成 changelog。

## 工作流程

1. 用 `git diff --staged`（或 `git diff`）查看本次改动内容
2. 判断改动类型，匹配下表的 type
3. 用一句话概括改动，写成 `<type>: <摘要>` 格式
4. 改动较大时补 body 说明「为什么改」（不是「改了什么」，diff 自己能说明）
5. 涉及多个关注点时，建议用户拆成多个 commit，而不是写一个超长 message
...
| fix | bug 修复 |
| docs | 文档 |
| style | 格式调整（不改逻辑，如缩进、分号） |
| refactor | 重构（既不是新功能也不是修 bug） |
| perf | 性能优化 |
| test | 测试相关 |
| chore | 构建、依赖、杂项 |

各 type 的详细判定边界见 references/conventions.md（拿不准时再读）。

## 规则

- 摘要用中文（匹配本项目惯例），祈使句，结尾不加句号
- 摘要 ≤ 50 字；body 每行 ≤ 72 字
- 不在 message 里写「Generated with AI」之类的痕迹
- body 解释「为什么」，不重复 diff 已说明的「改了什么」

## 示例（照着学）

单行（最常见）：
  feat: 新增「降级详解」博客文章
  fix: 修复限流计数器在窗口边界的临界突发问题
  docs: 更新部署指南，补充 Node 22 的说明

带 body（改动复杂时）：
  refactor: 重构熔断状态机的恢复逻辑

  原来的 Half-Open 直接全量放行，下游没恢复时会被再次压垮。
  改成「固定请求数试探」，探测成功才回 Closed，失败回 Open。
```

**references/conventions.md**（按需读取的细节）：

```markdown
# 约定式提交的边界判定

## feat vs fix
- feat：对用户可见的新能力（新接口、新功能、新选项）
- fix：修正了「本应正常但出了错」的行为

## refactor vs feat/fix
- refactor：既不增加能力，也不修复 bug，只是改善结构
- 边界：如果重构后行为变了（哪怕是修正了隐含 bug），优先用 fix

## BREAKING CHANGE
破坏性变更在 footer 标注：
  feat: 重构用户认证流程

  BREAKING CHANGE: 移除了 /api/v1/login，改用 /api/v2/auth
```

**这个示例体现了什么**：
- frontmatter 的 description 列举了触发措辞（「提交代码」「写个 commit」）
- 正文有清晰的**工作流程**（5 步）+ **规则** + **示例**
- 常用 type 速查表放正文（每次都要用），**详细边界判定拆到 references**（拿不准才读）——这正是渐进式披露
- 示例给了「单行」和「带 body」两种形态，模型照着学
...
① 自动触发（主要方式）
   ...
   例：用户说「帮我写个 commit」
       → Agent 看到 commit-message 的 description 里有「写个 commit」
       → 自动触发，加载 SKILL.md 正文

② 强制触发（调试/兜底）
   用斜杠命令直接指定，不依赖 description 匹配：
   /skill commit-message 帮我看下这次改动
...
加载 SKILL.md 正文（第 2 层）
  ↓ 读到「工作流程：先 git diff --staged」
调用工具：执行 git diff --staged      ← 工具调用
  ↓ 拿到改动内容
按 SKILL.md 的 type 速查 + 规则生成 message
  ↓ 改动复杂，需要确认边界
按需读取 references/conventions.md    ← 第 3 层（按需）
  ↓
输出：feat: 新增降级详解文章
       （交付给用户）
```
...
- 第 2 层（SKILL.md）告诉它「怎么做」
- 第 3 层（references）在「需要细节」时才读
- 工具调用（git diff）让它能「真正执行动作」
...
1. 自动触发测试：说一句能匹配 description 的话，看 Agent 有没有自动用上
   （比如直接说「帮我提交代码」，而不是「用 commit-message 技能」）

2. 强制触发测试：用 /skill <name> 显式调起，确认 Skill 内容能正确加载

3. 边界测试：故意说一句模糊的话（如「整理一下这次的改动」），
...
| 有完整示例可参考吗？ | 见 5.4 的「commit-message」技能——单文件 + references 拆分，五脏俱全 |
